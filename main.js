      ...
      if(status != "")
      {
        ...
        for (i = 0; i < objects.length; i++) {
          ...
          if (objects[i].label == objectName) {
            ...
            document.getElementById("objectStatus").innerHTML = objectName + "Objeto Encontrado";
            synth = window.speechSynthesis;
            utterThis = new SpeechSynthesisUtterance(objectName + " Encontrado");
            synth.speak(utterThis);
          }
          else {
            document.getElementById("objectStatus").innerHTML = objectName + "Objeto não Encontrado";
          }
        }
      }
}
